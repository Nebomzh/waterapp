...
public class ContactsActivity extends AppCompatActivity implements OnMapReadyCallback, GoogleMap.OnMapLongClickListener {
    ...
    private void placeInitialMarkerAndMove() {
        if (index == 0) { // Place a marker at the user's location
            ...
            LatLng savedLocation = new LatLng(ProblemaActivity.locations.get(index).getLatitude(), ProblemaActivity.locations.get(index).getLongitude());
    ...
    public void onMapLongClick(LatLng point) {
        ...
        ProblemaActivity.addLocation(addressName, point.latitude, point.longitude);
        ProblemaActivity.saveLocations();
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (id) {
            ...
            // Marker options
            case R.id.show:
                for (int i = 1; i < ProblemaActivity.locations.size(); i++) {
                    LatLng coordinates = new LatLng(ProblemaActivity.locations.get(i).getLatitude(), ProblemaActivity.locations.get(i).getLongitude());
                    mMap.addMarker(new MarkerOptions().position(coordinates).title(ProblemaActivity.places.get(i))
                            .icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_MAGENTA)));
                }
                break;
